...
class Base_Task(gym.Env):
    ...
    def get_obs(self):
        ...
        if self.data_type.get("actor_segmentation", False):
            ...
            if hasattr(self, 'target_objects') and self.target_objects:
                ...
                # Save current target mask separately for convenience
                if current_target and current_target in self.target_objects:
                    pkl_dic["current_target_mask"] = {}
                    for camera_name in target_masks.keys():
                        if current_target in target_masks[camera_name]:
                            pkl_dic["current_target_mask"][camera_name] = target_masks[camera_name][current_target]
    ...
    def set_current_target(self, target_name: str):
        """
        Set the current active target object for attention/mask focus.
